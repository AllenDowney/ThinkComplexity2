...
Suppose we start at node $s$.  We can mark $s$ as ``reachable'',
...
Initially the set, \py{reachable}, is empty, and we create a
...
\item If the node is already in \py{reachable}, we go back
to Step 1.

\item Otherwise, we add the node to \py{reachable} and add its
neighbors to the stack.

\end{enumerate}

When the stack is empty, we can't reach any more nodes, so we
break out of the loop and return \py{reachable}.
...
Initially, the stack contains node 0 and \py{reachable} is empty.
The first time through the loop, node 0 is added to \py{reachable}
...
in the stack, which is node 9.  So node 9 gets added to \py{reachable}
...
Create a file named \py{SmallWorldGraph.py} and define a class named
\py{SmallWorldGraph} that inherits from \py{RandomGraph}.

If you did Exercise~\ref{ex.randomgraph} you can use your own
\py{RandomGraph.py}; otherwise you can download mine from
\url{thinkcomplex.com/RandomGraph.py}.

\begin{enumerate}

\item Write a method called \py{rewire} that takes a probability {\tt
  p} as a parameter and, starting with a regular graph, rewires
the graph using Watts and Strogatz's algorithm.

\item Write a method called \py{clustering_coefficient} that
