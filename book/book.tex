...
{\small
Copyright \copyright ~2016 Allen B. Downey.
...
Permission is granted to copy, distribute, transmit and adapt
this work under a Creative Commons
Attribution-NonCommercial-ShareAlike 4.0 Unported International License:
...
the relationship the represent is asymmetric or symmetric.  In a road
map, you might represent a one-way street with a directed edge and a
two-way street with an undirected edge.  In some social networks,
like Facebook, friendship is symmetric: if $A$ is friends with $B$ then $B$
is friends with $A$.
But in Twitter, for example, the ``follows''
relationship is not symmetric; if $A$ follows $B$, that doesn't imply
that $B$ follows $A$.  So you might use undirected edges to represent
...
in the northeast United States with labels on the edges that
...
It is conventional to import NetworkX as {\tt nx}.  At this
point, {\tt G} is a {\tt DiGraph} object that contains no nodes
and no edges.  We can add nodes using the \verb"add_node" method:
...
Now we can use the {\tt nodes} method to get the list of nodes:
...
And we can use {\tt edges} to get a list of edges:
...
NetworkX provides several functions to represent graphs visually;
\verb"draw_circular" arranges the nodes in a circle and connects them
with edges:
...
And that's the code I used to generate Figure~\ref{chap02-1}.
{\tt COLORS} is a list of colors I like better than the defaults.
The option \verb"with_labels" causes the nodes to be labeled;
...
To generate Figure~\ref{chap02-2}, I started with a dictionary
...
Since this is an undirected graph, I instantiate {\tt nx.Graph}:

\begin{verbatim}
G = nx.Graph()
\end{verbatim}

Then I can use \verb"add_nodes_from" to iterate the keys of
{\tt pos} and add them as nodes:
...
Now I can use \verb"add_edges_from", which iterates the keys of
\verb"drive_times" and add them as edges:

\begin{verbatim}
G.add_edges_from(drive_times)
\end{verbatim}

Now instead of \verb"draw_circular", which arranges the nodes in 
circle, I'll use \verb"draw", which takes {\tt pos} as the second
parameter:
...
To add the edge labels, we use \verb"draw_networkx_edge_labels":
...
One of the more
interesting kinds is the \Erdos-\Renyi~model, studied by
Paul \Erdos 
and Afr\'{e}d \Renyi~in the 1960s.
...
random edges are added.  This transition is sometimes called
...
connectedness.  An undirected graph is {\bf connected} if there is a
path from every node to every other node.

In an ER graph, the critical value is
...
Now we can use \verb"all_pairs" to construct a complete graph:
...
\verb"make_complete_graph" takes the number of nodes, {\tt n}, and
returns a new {\tt Graph} with {\tt n} nodes and edges between all
pairs of nodes.
...
% TODO: Why is this a useful thing to check?

There is a simple algorithm to check whether a graph is connected.
You can start at any node and mark it as ``reachable''.  Then
mark its neighbors (two nodes are {\bf neighbors} if they are
connected by an edge.  Then mark the neighbor's neighbors, and so
on, until you can't reach any more nodes.  If all nodes are
marked, the graph is connected.
...
\verb"reachable_nodes" takes a {\tt Graph} and a starting node, {\tt
  start}, and returns the set of nodes that can be reached from {\tt
  start}.

Initially the set, {\tt reachable}, is empty, and we create a
list called {\tt stack} that will keep track of nodes we have
discovered but not yet processed.  Initially the stack contains
a single node, {\tt start}.
...
\item If the node is already in {\tt reachable}, we can go back
to Step 1.

\item Otherwise, we add the node to {\tt reachable} and add its
neighbors to the stack.  The {\tt neighbors} method returns a list
of neighbors of the given node.

\end{enumerate}

When the stack is empty, we can't reach any more nodes, so we
break out of the loop and return {\tt reachable}.
...
Initially, the stack contains node 0 and {\tt reachable} is empty.
The first time through the loop, node 0 is added to {\tt reachable}
and all the other nodes are added to the stack (since they are all
neighbors of node 0).

The next time through the loop, {\tt pop} returns the last element
in the stack, which is node 9.  So node 9 gets added to {\tt reachable}
...
$k$ times.  But each node is only processed once, so the process
has to terminate.

We can use \verb"reachable_nodes" to write \verb"is_connected":
...
\verb"is_connected" chooses a starting node arbitrarily by calling
\verb"nodes_iter", which returns an iterator object, and passing the
result to {\tt next}, which returns the first node.

{\tt reachable} gets the set of nodes that can be reached from
{\tt start}.  If the size of this set is the same as the size
...
The following generator function enumerates all possible edges and uses
a random number generator to choose which ones should be added to the graph:
...
The helper function {\tt flip} returns {\tt True} with the
given probability, {\tt p}, and {\tt False} with the complementary
probability {\tt 1-p}.
...
Finally, \verb"make_random_graph" generates and returns the 
ER graph $G(n, p)$.
...
Here's an example with {\tt p=0.5}:
...
Figure~\ref{chap02-4} shows the result.

\begin{figure}
\centerline{\includegraphics[width=3.5in]{figs/chap02-4.pdf}}
\caption{An ER graph with {\tt n=10} and {\tt p=0.3}.}
\label{chap02-4}
\end{figure}
