...
yields one edge at a time.  It uses the modulus operator, \py{%},
...
neighbors, the clustering coefficient is undefined.  The value
\py{np.nan} is a special floating-point value that represents ``not
a number''.
...
Now we are ready to replicate the WS experiment, which showed that for
...
Now we need to compute these values for a range of `p`.  I'll use
the NumPy function `logspace` again to compute `ps`:

\begin{code}
ps = np.logspace(-4, 0, 9)
\end{code}

For each value of `p`, I generate 3 random graphs and we'll average the
...
The result is a dictionary that maps from each value of `p` to
a list of `(mpl, cc)` pairs.
...
%# from  https://github.com/networkx/networkx/blob/master/networkx/algorithms/components/connected.py
