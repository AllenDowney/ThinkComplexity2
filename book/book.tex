...
I'll use \py{clustering_coefficient} from Section~\ref{clustering}
...
Alternatively, you could use \py{average_clustering}, which is provided
by NetworkX:
...
To compute the average path length, I'll use \py{path_lengths} from
...
\caption{PMF of degree in the Facebook network and in the WS model.}
\label{chap04-1}
\end{figure}
...
The mean degree in the Facebook network is 43.7; the mean degree in
...
Now we can make a \py{Pmf} that contains node degrees from the Facebook
data, and compute the mean and standard deviation:
...
In the Facebook network, there are many users with only 1 or 2 friends,
...
Under this transformation, the Facebook data fall approximately on a
...
of edges per node in the Facebook data.

\begin{figure}
\centerline{\includegraphics[width=5.5in]{figs/chap04-3.pdf}}
\caption{PMF of degree in the Facebook data and in the BA model,
on a log-log scale.}
\label{chap04-2}
\end{figure}
...
is 43.8, very close to the average degree in the Facebook data,
43.7.

And the standard deviation of degree is 40.9, which is a bit
less than in the Facebook data, 52.4, but it is much better
than what we got from the WS graph, 1.5.

Figure~\ref{chap04-3} shows the degree distributions for the
Facebook data and the BA model on a log-log scale.  The model
...
not even close to the value in the Facebook network, $0.606$.
So that's a problem.
...
\begin{table}[]
\centering
\caption{Characteristics of the Facebook data and two models.}
\label{table04-1}
\begin{tabular}{lrrr}
\hline
...
of `barabasi_albert_graph`, with some changes I made to
...
% BA data here http://www3.nd.edu/~networks/resources.htm
