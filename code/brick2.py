...
def recurse_levels(width, height):
    ...
    A configuration is a frozenset of seam locations.
    """
    # base case: if there's only one row, just enumerate the combos
    if height == 1:
        return bundle_combos(start=0, width=width, avoid=frozenset())
...
def bundle_combos(start, width, avoid, factor=1):
    ...
    avoid: frozenset of locations where you can't put a seam
...
def add_brick(end, combos):
    ...
    Returns a new list.
    """
    return [combo.union([end]) for combo in combos]
